...
/**
  \fn          int32_t sdsWrite (sdsId_t id, uint32_t time_slot, const void *buf, uint32_t buf_size)
...
/**
  \fn          int32_t sdsRead (sdsId_t id, uint32_t *time_slot, void *buf, uint32_t buf_size)
  \brief       Read entire data block along with its time slot information from the SDS stream opened in read mode.
  \param[in]   id             \ref sdsId_t handle to SDS stream
  \param[out]  time_slot      pointer to buffer for a time slot value
