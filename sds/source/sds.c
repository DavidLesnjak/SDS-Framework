...
// Data block header
typedef struct {
  uint32_t    time_slot;        // Time slot value
...
// Translate SDSIO error codes to SDS error codes.
static int32_t sdsTranslateIoErr (int32_t sdsio_err) {
  int32_t ret;
...
sdsId_t sdsOpen (const char *name, sdsMode_t mode, void *buf, uint32_t buf_size) {
  ...
  if (sdsLockAcquire(stream, SDS_OPEN_TOUT) == 0U) {
    // Timeout occurred while waiting for lock. Exit the function.
    return NULL;
  }
...
*/
int32_t sdsWrite (sdsId_t id, uint32_t time_slot, const void *buf, uint32_t buf_size) {
  sdsStream_t    *stream = id;
  int32_t         ret      = SDS_ERROR;
...
*/
int32_t sdsRead (sdsId_t id, uint32_t *time_slot, void *buf, uint32_t buf_size) {
  sdsStream_t *stream = id;
  int32_t      ret      = SDS_ERROR;
  int32_t      eos      = 0;
  int32_t      size;
    ...
    if (stream->head.data_size != 0U) {
      ...
      if (stream->head.data_size > buf_size) {
        ...
        if (stream->head.data_size > size) {
          ...
          if (time_slot != NULL) {
            *time_slot = stream->head.time_slot;
          }
